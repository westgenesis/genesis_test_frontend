<template>
  <div class="flex">
    <div
      ...
      class="m-[20px] w-[28rem]"
    >
      <div style="border-bottom: 1px solid #ddd; padding-bottom: 1rem">
    ...
    <div class="w-full h-[90%] pt-[2rem] overflow-scroll">
      <div
        v-show="currentType === 'requirement'"
        class="w-full pt-[2rem]"
        style="height: 90vh"
      >
        <div
          style="
        ...
        <div class="flex-container" style="margin-left: 1rem">
          <div class="flex-item">
            <strong>项目ID:</strong>
            {{ currentRequirement?.project?._id?.$oid }}
          </div>
          <div class="flex-item">
        ...
        <el-table
          :data="flattened_cases_req_paged"
          style="width: 100%"
        ...
        <div class="flex justify-center mt-4">
          <a-pagination
            v-model:current="currentPageReq"
            :total="totalItemsReq"
            :page-size="pageSizeReq"
            @change="handlePageChangeReq"
          />
        </div>
      </div>
      <div
        v-show="currentType === 'sub_requirement'"
        class="w-full pt-[2rem]"
        style="height: 90vh"
      >
        <div
        ...
        <div class="flex-container" style="margin-left: 1rem">
          <div class="flex-item">
            <strong>功能模块名称:</strong>
            {{ currentRequirement?.splitReq?.file_name?.replace(".docx", "") }}
          </div>
          <div class="flex-item">
            <strong>功能模块ID:</strong>
            {{ currentRequirement?.splitReq?.split_file_id }}
          </div>
          <div class="flex-item">
            <strong>当前版本:</strong>
            {{ currentRequirement?.splitReq?.version }}
          </div>
          <div class="flex-item">
            <strong>项目描述:</strong>
            {{ currentRequirement?.splitReq?.description || "无描述" }}
          </div>
        </div>
        <div
          style="
      ...
      >
        ...
        <div class="flex-container" style="margin-left: 1rem">
          <div class="flex-item">
            <strong>功能点名称:</strong>
            {{ currentRequirement?.splitCase?.testcase_name }}
          </div>
          <div class="flex-item">
            <strong>功能点ID:</strong>
            {{ currentRequirement?.splitCase?.testcase_id }}
          </div>
          <div class="flex-item">
            <strong>当前版本:</strong>
            {{ currentRequirement?.splitCase?.version }}
          </div>
          <div class="flex-item">
            <strong>功能点描述:</strong>
            {{ currentRequirement?.splitCase?.description || "无描述" }}
          </div>
        </div>
        <div
          style="
        ...
        >
          <div class="flex">
            <a-input-search
              ...
              @search="onSplitCaseSearch"
            />
            <a-select
              v-model:value="splitCaseSelect"
      ...
      <div v-show="currentType === 'testcase'" style="height: 90vh">
        <a-form :model="form" layout="vertical">
          ...
          <a-form-item label="初始条件信号">
            <a-textarea
              v-model:value="form.pre_condition_signal"
          ...
          <a-form-item label="触发条件信号">
            <a-textarea
              v-model:value="form.action_signal"
          ...
          <a-form-item label="预期结果信号">
            <a-textarea
              v-model:value="form.result_signal"
              placeholder="请输入预期结果"
              :rows="3"
            />
          </a-form-item>
          <a-form-item>
            ...
            >
              ...
              <div style="color: red">
                提示：当前为V{{ form.version }}版本 保存后版本新增
              </div>
            </div>
          </a-form-item>
        </a-form>
      ...
      >
        <a-form :model="newForm" layout="vertical">
          ...
          <a-form-item label="初始条件">
            <a-textarea
              v-model:value="newForm.pre_condition"
              placeholder="请输入初始条件"
              :rows="3"
            />
          </a-form-item>
  ...
  <a-modal
    v-model:visible="selectBelongsToModalVisible"
    title="选择所属对象"
    @ok="handleSelectBelongsToOk"
  >
...
const handleRemoveSignal = ({ type, signal }) => {
  if (type === "preCondition") {
    need_fill_result.value.pre_condition_signal =
      need_fill_result.value.pre_condition_signal.filter(
        (s) => s.name + s.value !== signal
      );
  } else if (type === "action") {
    need_fill_result.value.action_signal =
      need_fill_result.value.action_signal.filter(
        (s) => s.name + s.value !== signal
      );
  } else if (type === "result") {
    need_fill_result.value.result_signal =
      need_fill_result.value.result_signal.filter(
        (s) => s.name + s.value !== signal
      );
  }
...
const refreshUseCase = async () => {
  ...
  const splitReq = req?.split_files.find(
    (x) => x.split_file_id === split_file_id
  );
  const splitCase = splitReq?.split_case.find(
    (x) => x.testcase_id === split_case_id
  );
  if (splitCase) {
...
const testcasesForSplitCase = computed(() => {
  return (currentRequirement.value?.splitCase?.testcases || [])
    .filter((x) => x.testcase_name.includes(searchForSplitCase.value))
    .filter((x) => x.type.includes(splitCaseSelect.value));
});
...
const onSelect: TreeProps["onSelect"] = (_, info) => {
  if (info?.node?.type === "requirement") {
    ...
    const flattened = split_cases.reduce(
...
const handleSave = async () => {
  ...
  return http.post("/api/modify_testcase", params).then(async (response) => {
    if (response.status === "OK") {
      ...
      const result = await refreshAllProjects();
      const currentTestcase = response.testcases.find(
        (testcase) => testcase.testcase_id === params.testcase_id
      );
...
const handleNewSave = async () => {
  ...
  delete params.name;
  return http
    .post("/api/create_testcase", params)
    .then(async (response) => {
      if (response) {
        ElMessage.success("保存成功");
        currentRequirement.value.splitCase.testcases = response.testcases;
...
const handleExport = () => {
  ...
  http
    ...
    .then((response) => {
      ...
      link.setAttribute(
        "download",
        currentRequirement.value.splitCase.testcase_id + ".xlsx"
      );
...
const export_by_sub_requirement = () => {
  ...
  http
    ...
    .then((response) => {
      ...
      link.setAttribute(
        "download",
        currentRequirement.value.splitReq.split_file_id + ".xlsx"
      );
      document.body.appendChild(link);
      link.click();
    })
...
const export_by_requirement = () => {
  ...
  http
    .post(
      "/api/export_by_cases",
      { cases: flattened_cases_req.value },
      { responseType: "blob" }
    )
    .then((response) => {
      const blob = new Blob([response as any], {
      ...
      link.setAttribute(
        "download",
        currentRequirement.value.req.name + ".xlsx"
      );
      document.body.appendChild(link);
      link.click();
    })
