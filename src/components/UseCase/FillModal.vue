      ...
      <div class="w-1/3 pr-4">
        <div class="signals-container">
          <div v-if="preConditionSignals.length">
            <h3>初始条件信号</h3>
            <div
              v-for="signal in preConditionSignals"
              :key="signal.name + signal.value"
              @click="handlePreConditionSignalChange(signal)"
              :class="[
                'signal-item',
                { selected: selectedSignal === signal.name + signal.value },
              ]"
            >
          ...
          <div v-if="actionSignals.length">
            <h3>触发条件信号</h3>
            <div
              v-for="signal in actionSignals"
              :key="signal.name + signal.value"
              @click="handleActionSignalChange(signal)"
              :class="[
                'signal-item',
                { selected: selectedSignal === signal.name + signal.value },
              ]"
          ...
          <div v-if="resultSignals.length">
            <h3>预期结果信号</h3>
            <div
              v-for="signal in resultSignals"
              :key="signal.name + signal.value"
              @click="handleResultSignalChange(signal)"
              :class="[
                'signal-item',
                { selected: selectedSignal === signal.name + signal.value },
              ]"
      ...
      <div class="w-2/3">
        ...
        <a-form :model="form" layout="vertical" v-if="activeTab === 'action'">
          ...
          <a-form-item label="动作描述" name="description">
            <a-input
              v-model:value="form.actionDescription"
              placeholder="请输入动作描述"
            />
          </a-form-item>
          <a-form-item label="动作执行路径" name="exec_path">
            <a-input
              v-model:value="form.execPath"
              placeholder="请输入动作执行路径"
            />
          </a-form-item>
          <a-form-item label="路径参数" name="path_parameter">
            <a-input
              v-model:value="form.pathParameter"
              placeholder="请输入路径参数"
            />
          </a-form-item>
        ...
        <div class="flex justify-end mt-4" v-if="activeTab === 'action'">
          <a-button type="primary" @click="handleCreateAction"
            >添加元动作</a-button
          >
        </div>
        <a-form
        ...
        >
          ...
          <a-form-item
            v-if="formData.is_file === '动作组合文件'"
            label="动作组合文件"
            name="combination_file"
          >
            <el-upload
              ref="uploadRef"
              :auto-upload="false"
          ...
          >
            ...
            >
              ...
              <a-select
                :value="formData.steps[index]._id"
                @change="
                  (v) => {
            ...
            <a-button type="dashed" @click="addStep" class="w-[200px]"
              >添加步骤</a-button
            >
          </a-form-item>
        </a-form>
        <div
          ...
          v-if="activeTab === 'combination'"
        >
          <a-button class="mr-[2rem]" type="primary" @click="handleOk"
            >添加动作组合</a-button
          >
        </div>
      </div>
...
watch(
  ...
  ([preCondition, action, result]) => {
    const selectedSignalValue =
      props.preConditionSignals.find(
        (signal) => signal.name + signal.value === preCondition
      ) ||
      props.actionSignals.find(
        (signal) => signal.name + signal.value === action
      ) ||
      props.resultSignals.find(
        (signal) => signal.name + signal.value === result
      );
    ...
    if (selectedSignalValue) {
      ...
      selectedSignal.value =
        selectedSignalValue.name + selectedSignalValue.value;
    }
  }
);
...
const handleCreateAction = async () => {
  ...
  try {
    ...
    if (resp.status === "ok") {
      ...
      // 如果所有信号都已创建，显示成功提示
      if (
        props.preConditionSignals.length === 0 &&
        props.actionSignals.length === 0 &&
        props.resultSignals.length === 0
      ) {
        emit("update:visible", false);
      }
    } else {
...
const handleTabChange = (tabKey) => {
  ...
  if (selectedPreConditionSignal.value) {
    selectedSignalValue = props.preConditionSignals.find(
      (signal) =>
        signal.name + signal.value === selectedPreConditionSignal.value
    );
  } else if (selectedActionSignal.value) {
    selectedSignalValue = props.actionSignals.find(
      (signal) => signal.name + signal.value === selectedActionSignal.value
    );
  } else if (selectedResultSignal.value) {
    selectedSignalValue = props.resultSignals.find(
      (signal) => signal.name + signal.value === selectedResultSignal.value
    );
  }
...
const onBeforeUpload = async (file) => {
  ...
  const info = new Blob([JSON.stringify({ category: "new_file" })]);
...
const updateAllowedMethods = () => {
  ...
  if (actionType && belongsTo) {
    ...
    if (
      actionType === "In" &&
      (belongsTo === "Vector_IO" ||
        belongsTo === "Vector_CAN" ||
        belongsTo === "Vector_LIN")
    ) {
      form.allowedMethods = "set";
    } else if (
      actionType === "Out" &&
      ...
      actionType === "In" &&
      (belongsTo === "dSpace_IO" ||
        belongsTo === "dSpace_CAN" ||
        belongsTo === "dSpace_LIN")
    ) {
