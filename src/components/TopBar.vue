<template>
  <div
    v-if="!isLoginOrRegistry"
    class="minh-[60px] flex bg-custom-bg justify-between items-center"
  >
    <div class="flex items-center">
      ...
      <div class="h-full flex items-center">
        <div
          ...
          :key="item.name"
          class="p-[10px] topBarItem cursor-pointer"
          :class="{ 'bg-white': item.name === selectedItem }"
          @click="handleTopBarClick(item)"
        >
    ...
    <a-dropdown>
      ...
      <template #overlay>
        <a-menu>
          <a-menu-item @click="backToLogin"> 退出登录 </a-menu-item>
        </a-menu>
      </template>
    </a-dropdown>
  </div>
