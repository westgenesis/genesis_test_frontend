    ...
    <a-form :model="formData" :rules="rules" layout="vertical" ref="formRef">
        ...
        <div v-for="(value, index) in formData.values" :key="index">
            <a-card class="mb-[20px]" style="background-color: rgb(236 236 236 / 34%)">
                <a-row :gutter="[16, 16]">
                    <a-col :span="12">
                        <a-form-item label="状态值" :name="['values', index, 'status']" :rules="{
                            required: true,
                            message: '状态值不可为空',
                            trigger: 'change',
                        }">
                            <a-input v-model:value="value.status" placeholder="请输入值名称" />
                        </a-form-item>
                    </a-col>

                    <a-col :span="12">
                        <a-form-item label="状态描述" :name="['values', index, 'description']" :rules="{
                            required: true,
                            message: '状态描述不可为空',
                            trigger: 'change',
                        }">
                ...
                <a-row :gutter="[16, 16]" align="middle">
                    <a-col :span="7">
                        <a-form-item label="VT信号" :name="['values', index, 'vt_signal']" :rules="{
                            required: true,
                            message: 'VT信号不可为空',
                            trigger: 'change',
                        }">
                    ...
                    <a-col :span="7">
                        <a-form-item label="符号" :name="['values', index, 'relation']" :rules="{
                            required: true,
                            message: '符号不可为空',
                            trigger: 'change',
                        }">
                    ...
                    <a-col :span="7">
                        <a-form-item label="取值" :name="['values', index, 'value']" :rules="{
                            required: true,
                            message: '取值不可为空',
                            trigger: 'change',
                        }">
