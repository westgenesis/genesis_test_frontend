<template>
  <div
    class="max-h-[70vh] min-h-[50vh] overflow-hidden overflow-y-auto pr-[30px]"
  >
    ...
    <div class="m-[32px]">
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'name'">
            <span>{{
              record.displayName || record.name || record.status
            }}</span>
          </template>
          ...
          <template v-if="column.key === 'action'">
            <template v-if="!record.values">
              <a-button type="link" size="small" @click="select(record, 'IO')"
                >选择</a-button
              >
            </template>
          </template>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'action'">
            <template v-if="!record.values || record.values.length === 0">
              <a-button type="link" size="small" @click="select(record, 'CAN')"
                >选择</a-button
              >
            </template>
          </template>
      ...
      <a-table
        ...
        bordered
        :data-source="combinationPagedDataSource"
        size="middle"
        :pagination="false"
        v-if="activeTab === '3'"
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'action'">
            <a-button type="link" size="small" @click="select(record, 'COM')"
              >选择</a-button
            >
          </template>
        </template>
...
function select(record, type) {
  const param = {
    ...
    type,
    isChild: record.values == undefined,
  };
