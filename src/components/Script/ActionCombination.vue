<template>
  ...
  <div class="m-[32px]">
    <div class="mt-[20px] mb-[20px] flex justify-end">
      <a-button type="primary" @click="showDrawer" class="mr-[2rem]"
        >添加动作组合</a-button
      >
    </div>
    ...
    >
      ...
      <div slot="footer" class="flex justify-end">
        <a-button class="mr-[2rem]" type="primary" @click="handleClose"
          >关闭</a-button
        >
        <a-button
          class="mr-[2rem]"
    ...
    >
      <a-form :model="editFormData" :rules="rules" layout="vertical">
        ...
        <a-form-item
          v-if="editFormData.is_file === '动作组合文件'"
          label="动作组合文件"
          name="combination_file"
        >
          <el-upload
        ...
        <a-form-item
          v-if="editFormData.is_file === '元动作'"
          label="动作组合内容"
          name="exec_path"
        >
          <div
            v-for="(step, index) in editFormData.steps"
            :key="index"
            class="mb-[10px]"
          >
            <span class="mr-[10px]">Step {{ index + 1 }}</span>
            <a-select
              :value="editFormData.steps[index]._id"
              @change="
                (v) => {
                  handleEditChange(v, index);
                }
              "
              placeholder="请选择动作"
              class="w-[200px]"
            ...
            <a-button
              type="link"
              size="small"
              @click="deleteEditStep(index)"
              class="ml-[10px]"
            >
              <delete-outlined />
            </a-button>
          </div>
          <a-button type="dashed" @click="addEditStep" class="w-[200px]"
            >添加步骤</a-button
          >
        </a-form-item>
      </a-form>
      <div slot="footer" class="flex justify-end">
        <a-button class="mr-[2rem]" type="primary" @click="handleEditClose"
          >关闭</a-button
        >
        <a-button
          class="mr-[2rem]"
...
const handleEditOk = async () => {
  ...
  try {
    ...
    const resp = await http.put(
      `/api/action_combinations/${editFormData._id}`,
      payload
    );
    editVisible.value = false;
    fetchActionCombinations();
