<template>
  <a-modal
    :visible="selectBelongsToModalVisible"
    title="脚本信息"
    @ok="verify"
    @cancel="emit('cancel')"
  >
  ...
  >
    <a-result status="warning" title="警告提示">
      <template #extra>
        <div>
          所选的用例中，存在信号缺失的情况，合并生成的脚本文件不可用，您可以选择以下操作：
        </div>
        ...
        <div class="mt-[20px]">
          ...
          <a-button
            type="primary"
            class="ml-[20px]"
            @click="handleSelectBelongsToOk('skip')"
            >跳过缺失继续生成</a-button
          >
          <a-button type="primary" class="ml-[20px]" @click="go"
            >跳转动作库</a-button
          >
...
onMounted(() => {
  ...
  const _minRequiredCANoeVersion =
    localStorage.getItem("_minRequiredCANoeVersion") || "";
...
const verify = () => {
  ...
  localStorage.setItem(
    "_minRequiredVersion",
    selectForm.value.minRequiredVersion
  );
  localStorage.setItem(
    "_minRequiredCANoeVersion",
    selectForm.value.minRequiredCANoeVersion
  );

  http
    .post("/api/batch_verify_generate_script_file", {
      data: [props.rowData],
      ...selectForm.value,
    })
    .then(
