<template>
  <a-tabs v-model:activeKey="activeTab">
    ...
    <a-tab-pane key="points" tab="正向测试用例"></a-tab-pane>
    ...
    <div style="width: 100%">
      ...
      >
        ...
        <el-table-column label="操作" :width="100">
          <template #default="scope">
            <el-button type="text" @click="handleGeneralization(scope.row)"
              >泛化反向用例</el-button
            >
          </template>
        </el-table-column>
...
const fetchData = () => {
  return http
    .post("/api/query_split_file_by_id", {
...
const clickTitle = (row) => {
  props.selectNodeByKey(
    [project_id.value, req_id.value, split_file_id.value, row.testcase_id].join(
      "-"
    )
  );
};
...
// 处理泛化反向用例
const handleGeneralization = (row) => {
  ElMessageBox.confirm("确认泛化反向用例？", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).then(() => {
    const { testcases, status, ...others } = row;
    const params = {
      points: [
        {
          ...others,
