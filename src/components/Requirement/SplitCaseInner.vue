<template>
  <a-tabs v-model:activeKey="activeTab">
    ...
    <a-tab-pane key="testcase_table" tab="反向测试用例"></a-tab-pane>
  </a-tabs>
...
// 处理泛化反向用例
const handleGeneralization = () => {
  ElMessageBox.confirm("确认泛化反向用例？", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  }).then(() => {
    const { testcases, status, ...others } = form.value;
    const params = {
      points: [
        {
          ...others,
          split_case_id: form.value.testcase_id,
          split_file_id: split_file_id.value,
          project_id: project_id.value,
          req_id: req_id.value,
        },
    ...
    http
      .post("/api/generate_generalized_testcase_by_points", params)
      .then((res) => {
        if (res) {
          ElMessage.success("泛化反向用例成功");
          refreshAllProjects();
        } else {
          ElMessage.error("泛化反向用例失败");
        }
      });
  });
};
