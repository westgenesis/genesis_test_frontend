    ...
    <div class="w-full h-[90%] pt-[2rem] overflow-scroll">
      ...
      >
        ...
        <div class="w-full flex justify-end mr-[2rem] mb-[1rem]">
          <a-button type="primary" @click="refreshModule" class="mr-[2rem]"
            >刷新</a-button
          >
      ...
      >
        ...
        <div style="width: 100%">
          ...
          <el-table
            :data="currentRequirement?.splitReq?.split_case"
            style="width: 100%"
...
const handleNewSave = async () => {
  ...
  delete params.name;
  return http
    .post("/api/create_split_case", params)
    .then(async (response) => {
      if (response) {
        ElMessage.success("保存成功");
        const res = await refreshAllProjects();
        const currentProject = res.find(
          (x) => x._id.$oid === newForm.value.project_id
        );
        const currentReq = currentProject.requirement_files.find(
          (x) => x.req_id === newForm.value.req_id
        );
        const currentSplitReq = currentReq.split_files.find(
          (x) => x.split_file_id === newForm.value.split_file_id
        );
        currentRequirement.value.splitReq.split_case =
          currentSplitReq.split_case;
        onDrawerClose();
        refreshCases();
      } else {
