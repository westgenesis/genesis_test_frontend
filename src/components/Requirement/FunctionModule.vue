<template>
  <div class="flex">
    <div
      ...
      class="m-[20px] mb-[0] w-[400px] flex flex-col"
    >
    ...
    <div class="w-full pt-[2rem] overflow-scroll">
      <div
        v-show="currentFile && currentType === 'sub_requirement'"
      ...
      >
        ...
        <div class="flex mt-[2rem] mb-[1rem]">
          <div class="w-[200px] ml-[1rem]">功能模块名称</div>
          <a-input
          ...
          <div class="w-[200px] ml-[1rem]">
            V{{ currentFile?.splitReq?.version }}
          </div>
        </div>
        ...
        <div
          ...
          "
        >
          提示：当前为V{{ currentFile?.splitReq?.version }}版本 由文档{{
            (currentFile?.req?.name || "").split("/")[1]
          }}
          v1版本生成 保存后版本新增
        </div>
      </div>
      ...
      >
        ...
        <div class="w-full flex justify-end mr-[2rem] mb-[1rem]">
          <a-button type="primary" @click="openAddDrawer" class="mr-[2rem]"
            >新建功能模块</a-button
          >
          <a-button type="primary" @click="handleDelete" class="mr-[2rem]"
            >删除</a-button
          >
    ...
    HTMLtoDOCX(documentWordEdit.quill.getSemanticHTML()).then((data) => {
      ...
      handler.UploadFile(formData).then((res) => {
        if (res) {
          ...
          currentFile.value.splitReq.version = (
            Number(currentFile.value.splitReq.version) + 0.1
          ).toFixed(1);
          refreshAllProjects();
        } else {
